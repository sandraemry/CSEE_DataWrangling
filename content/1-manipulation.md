...
👉 [Slides from the workshop](https://docs.google.com/presentation/d/1XmZUMXx7ZwPMYdWfyAyOIMlvNB2waFWXUPvmDOhkEek/edit?usp=sharing)
...
For example, let's calculate the ratio of bill length to bill depth 
```r
penguins %>% 
...
Sometimes you might want to add your calculated row to the same DF, i.e.g not get rid of any rows or columns. For example, if we wanted to filter based on the mean value that we just calculated. We can group_by and mutate instead. 
...
What about the other way around? How many rows and columns do we expect here?
```
left_join(People, Batting, by = "playerID") %>% dim()
```

It's helpful to tell R what you are expecting, and R will tell you if you are wrong.
```r
...
1. Create a smaller dataset that only includes species, island, bill_length_mm, bill_depth_mm, and body_mass_g. Call this subset penguins_small 

2. Rename species to common_name using the full dataset (penguins)

3. Filter for Gentoo penguins only, on the island of Biscoe. Call this subset gentoo_biscoe

4. Create a new variable for bill ratio (length / depth) using the full dataset. 

5. Sort the data by body_mass_g, from heaviest to lightest. 

6. Calculate the average body mass and bill length for each species in the full dataset. Save this as penguins_summary

7. Count how many penguins are recorded for each island.

8. Add the species average body mass and back to the main dataset. (Try using a join for this task)

9. Convert bill length and depth to a long format.
